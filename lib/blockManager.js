    ...
    const done = (err) => {
        ...
        if (totalPayments == 0) {
            ...
            global.coinFuncs.getLastBlockHeader(function (err, body) {
                ...
                const tipHeight = body.height;
                return preCalculatePPLNSPayments(block_hex, tipHeight, block_difficulty, is_store_dump, done_callback);
            });
            return;
        } else {
    ...
    for (const block of blockList) {
        ...
        if (error) {
            ...
            if (
                ...
                console.log(`Invalidating block ${block.height} (${block.anchor_height}) due to being an orphan block`);
        ...
        const remoteHeader = result.block_header;

        if (remoteHeader.hash !== block.hash) {
        ...
        // }
        //
        // if (tipHeight - block.height > global.config.payout.blocksRequired && (!is_pplns_block || block.pay_ready === true)) {
...
async function unlockAuxBlocks() {
    ...
    const lastHeader = await util.promisify((cb) => global.coinFuncs.getLastBlockHeader(cb))();
    ...
    for (const block of blockList) {
        ...
        switch (block.poolType) {
            case global.protos.POOLTYPE.PPLNS: {
                if (!(block.hash in payReadyBlockHashCalc) && !block.pay_ready) {
                    debug(`Processing PPLNS payment for block ${block.height}`);
                    payReadyBlockHashCalc[block.hash] = 1;
...
function Payout(blockHash, coin, poolType, paymentAddress) {
    ...
    this.payment_address = aux.tryExtractAddress(paymentAddress, coin);
    this.payment_id = null;
    this.bitcoin = 0;
    this.amount = 0;

    if (!this.payment_address) {
        throw new Error('No payment address provided for Payout!');
    }
...
async function processPplnsPayments(block) {
    ...
    let blockHeight = block.anchor_height;
    let totalShares = 0;
    let firstShareTime;
    let lastShareTime;
    // if (totalPaid >= rewardTotal) return;
    ...
    // console.log(feePayout, coinDevPayout, poolDevPayout);
...

    {
        ...
        while (totalPaid < rewardTotal && blockHeight > 0) {
            debug(
                `Checking shares for Height: ${blockHeight} Total Payment: ${totalPaid} Amount Paid: ${rewardTotal} Amount Total: ${totalPaid}`,
            );
            ...
            for (let found = !!key; found; found = cursor.goToNextDup()) {
                ...
                debug(
                    `Adding payout amounts [${currentPayout.payment_address}]: ${amount}, poolDev = ${poolDonation}, coinDev = ${devDonation}`,
                );

                if (!firstShareTime) firstShareTime = share.timestamp;
                if (totalPaid < rewardTotal) lastShareTime = share.timestamp;

                function addPayment(payout, amount) {
                    if (amount === 0) return;
                    if (totalPaid >= rewardTotal) return;
                    totalShares += amount;
                ...
                const remainingFees = totalFees - devDonation - poolDonation;
                addPayment(feePayout, remainingFees);
            }
    ...
    console.log(
        `Paying out to ${finalPayouts.length} addresses, a total of ${totalPayments} for block ${block.hash} (${coin})`,
    );
    ...
    Object.keys(finalPayouts).forEach(function (key) {
        if (finalPayouts[key].amount) {
            finalPayouts[key].amount = finalPayouts[key].amount / payWindow;
...
const SLEEP_INTERVAL = 20 * 1000;
async function startBlockUnlocker() {
    while (true) {
        try {
            // await unlockBlocks();
            await unlockAuxBlocks();
        } catch (err) {
