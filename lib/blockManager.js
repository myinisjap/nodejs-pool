        ...
        if (balance_sum !== prev_balance_sum) {
            console.log(
                'Total balance changed from ' +
                    global.support.coinToDecimal(prev_balance_sum) +
                    ' to ' +
...
async function unlockBlocks() {
    ...
    for (const block of blockList) {
        if (tipHeight - block.height <= global.config.payout.blocksRequired) {
            console.log(
                `Block ${block.height} (${block.hash}) does not have the required number of confirmations for processing (${global.config.payout.blocksRequired}). Skipped.`,
            );
            return;
        }
    ...
    for (const block of blockList) {
        ...
        switch (block.poolType) {
            case global.protos.POOLTYPE.PPLNS: {
                ...
                if (block.pay_ready && tipHeight - block.height > global.config.payout.blocksRequired) {
                    blockPayments(block, function () {});
                }
...
async function processPplnsPayments(block) {
    ...
    console.log(
        `Paying out to ${Object.keys(finalPayouts).length} address(es), a total of ${totalPayments} for block ${
            block.hash
        } (${coin})`,
    );
