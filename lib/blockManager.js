...
let createBlockBalanceQueue = async.queue(function (task, callback) {
    global.mysql
        .query(
            'REPLACE INTO block_balance (hex, payment_address, payment_id, amount) VALUES (?, ?, ?, ?)',
            [task.hex, task.payment_address, task.payment_id, task.amount]
        )
        .then(function (result) {
            if (
                !result.hasOwnProperty('affectedRows') ||
                (result.affectedRows != 1 && result.affectedRows != 2)
            ) {
...
let createBalanceQueue = async.queue(function (task, callback) {
    ...
    if (payment_id !== null) {
        query =
            'SELECT id FROM balance WHERE payment_address = ? AND payment_id = ? AND pool_type = ? AND bitcoin = ?';
    }
    let cacheKey = payment_address + pool_type + bitcoin + payment_id;
    debug('Processing a account add/check for:' + JSON.stringify(task));
    global.mysql
        .query(query, [payment_address, payment_id, pool_type, bitcoin])
        .then(function (rows) {
            if (rows.length === 0) {
                global.mysql
                    .query(
                        'INSERT INTO balance (payment_address, payment_id, pool_type, bitcoin) VALUES (?, ?, ?, ?)',
                        [payment_address, payment_id, pool_type, bitcoin]
                    )
                    .then(function (result) {
...
let balanceQueue = async.queue(function (task, callback) {
    ...
    let payment_id = null;
    if (
        typeof task.payment_id !== 'undefined' &&
        task.payment_id !== null &&
        task.payment_id.length > 10
    )
    ...
    async.waterfall(
        [
            ...
            function (balance_id, intCallback) {
                ...
                global.mysql
                    .query('UPDATE balance SET amount = amount+? WHERE id = ?', [
                        amount,
                        balance_id,
                    ])
                    .then(function (result) {
        ...
        for (
            ...
            cursor.getCurrentBinary(function (key, data) {
                ...
                if (shareData.poolType === global.protos.POOLTYPE.PPS) {
                    ...
                    paymentData[userIdentifier].amount =
                        paymentData[userIdentifier].amount + amountToPay;
                    let donations = 0;
                    ...
                    if (global.config.payout.poolDevDonation > 0) {
                        let poolDevDonation =
                            feesToPay * (global.config.payout.poolDevDonation / 100);
...
async function preCalculatePPLNSPayments(
    block_hex,
    block_height,
    block_difficulty,
    is_store_dump,
    ...
    console.log(
        'Performing PPLNS reward pre-calculations of block ' +
            block_hex +
            ' on (anchor) height ' +
            block_height
    );
    ...
    const process = (callback) => {
        ...
        let cursor = new global.database.lmdb.Cursor(txn, global.database.shareDB);
        for (
            let found = cursor.goToRange(blockCheckHeight) === blockCheckHeight;
            found;
            found = cursor.goToNextDup()
        ) {
            cursor.getCurrentBinary(function (key, data) {
                ...
                debug(
                    `Processing share ${shareData.paymentAddress} (poolType = ${shareData.poolType}`
                );
                ...
                const feesToPay =
                    ...
                    (shareData.bitcoin === true
                        ? amountToPay * (global.config.payout.btcFee / 100)
                        : 0);
                ...
                addPayment(
                    global.config.payout.feeAddress,
                    feesToPay - devDonation - poolDevDonation
                );
...
function doPPLNSPayments(block_hex, block_reward, unlock_callback) {
    console.log(
        'Performing PPLNS payout of block ' +
            block_hex +
            ' with value ' +
            global.support.coinToDecimal(block_reward)
    );
    global.mysql.query('SELECT SUM(amount) as amt FROM balance').then(function (rows) {
        if (typeof rows[0] === 'undefined' || typeof rows[0].amt === 'undefined') {
        ...
        global.mysql
            .query('SELECT payment_address, payment_id, amount FROM block_balance WHERE hex = ?', [
                block_hex,
            ])
            .then(function (rows) {
                if (rows.length) {
...
function calculateSoloPayments(blockHeader) {
    ...
    for (
        ...
        cursor.getCurrentBinary(function (key, data) {
            ...
            let rewardTotal = blockHeader.reward;
            if (
                shareData.poolType === global.protos.POOLTYPE.SOLO &&
        ...
        case global.protos.POOLTYPE.PPLNS:
            global.coinFuncs.getBlockHeaderByHash(block.hash, function (err, header) {
                if (
                    ...
                    console.error(
                        "Can't get correct block header by hash " + block.hash.toString('hex')
                    );
                    global.support.sendEmail(
                        global.config.general.adminEmail,
...
async function unlockBlocks() {
    ...
    debug(`Found ${blockList.length} valid locked block(s)`);
    ...
    for (const block of blockList) {
        ...
        debug(
            ...
            (is_pplns_block = ${is_pplns_block.toString()}, hash = ${block.hash}, \
             pay_ready = ${block.pay_ready})`
        );

        let [err, resp] = await util
            .promisify((h, cb) => global.coinFuncs.getBlockHeaderByID(h, cb))(block.height)
            .then((r) => [null, r])
            .catch((err) => [err, null]);
        if (err) {
        }
        debug(`Daemon responsed with block ${JSON.stringify(resp)}`);
...
async function startBlockUnlocker() {
    while (true) {
        try {
            debug('Block unlocker ready to work');
            await unlockBlocks();
        } catch (err) {
