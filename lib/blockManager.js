...
let blockQueue = async.queue(function (task, callback) {
    ...
    global.support.rpcDaemon('getblockheaderbyheight', {"height": task.blockID}, function (body) {
        if (!body.result) {
            console.error("getblockheaderbyheight error: ", body);
            throw body;
        }
...
let createBalanceQueue = async.queue(function (task, callback) {
    ...
    global.mysql.query(query, [payment_address, payment_id, pool_type, bitcoin]).then(function (rows) {
        if (rows.length === 0) {
            global.mysql.query("INSERT INTO balance (payment_address, payment_id, pool_type, bitcoin, coin) VALUES (?, ?, ?, ?, ?)", [payment_address, payment_id, pool_type, bitcoin, coin]).then(function (result) {
...
function blockUnlocker() {
    ...
    console.log("Unlocking " + blockList.length + " block(s)");
    // Todo: Implement within the coins/<coin>.js file.
