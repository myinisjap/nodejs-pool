...
const { tryParseMergeMiningAddress, compareGte } = require('../common');
...
// Merge mining extensions key name
const MMPROXY_EXTENSIONS_KEY_NAME = '_mmproxy_ext';
...
function Coin(data) {
    ...
    instanceId.writeUInt32LE(
        ((global.config.pool_id % (1 << 10) << 22) + (process.pid % (1 << 22))) >>> 0
    );
    ...
    this.getPortBlockHeaderByHash = function (port, blockHash, callback) {
        global.support.rpcPortDaemon(port, 'getblock', { hash: blockHash }, function (body) {
            ...
            const miner_tx_hash =
                body.result.miner_tx_hash || body.result.block_header.miner_tx_hash;

            if (body.result.hasOwnProperty('miner_tx_hash')) {
                global.support.rpcWallet(
    ...
    this.getPortLastBlockHeader = function (port, callback) {
        global.support.rpcPortDaemon(port, 'getlastblockheader', [], function (body) {
            if (typeof body !== 'undefined' && body.hasOwnProperty('result')) {
                ...
                console.error('Last block header invalid: ' + JSON.stringify(body));
                return callback(true, body);
            }
        });
    };
    ...
    this.getPortBlockTemplate = function (port, callback) {
        const reserve_size =
            port in mm_port_set ? mm_nonce_size + pool_nonce_size : pool_nonce_size;
    ...
    this.BlockTemplate = function (template) {
        ...
        let merge_mining_ext = template[MMPROXY_EXTENSIONS_KEY_NAME] || {};

        this.difficulty = template.difficulty;
        if (merge_mining_ext.difficulties) {
            this.tariDifficulty = merge_mining_ext.difficulties.xtr;
            this.moneroDifficulty = merge_mining_ext.difficulties.xmr;
        }
        ...
        const template_hex =
            template.port in mm_port_set ? extra_nonce_mm_template_hex : extra_nonce_template_hex;
        ...
        if (!('prev_hash' in template)) {
            ...
            const prev_hash_start = global.coinFuncs.blobTypeRaven(port2blob_num[this.port])
                ? 4
                : 7;
        ...
        this.isDifficultyReached = (diff) => compareGte(diff, this.difficulty);
    };
    ...
    this.convertAlgosToCoinPerf = function (algos_perf) {
        ...
        if ('cn/gpu' in algos_perf)
            coin_perf['RYO'] = coin_perf['CCX'] = coin_perf['XEQ'] = algos_perf['cn/gpu'];
    ...
    this.algoCheck = function (algos) {
        ...
        return (
            'algo array must include at least one supported pool algo: [' +
    ...
    this.slowHashAsync = function (convertedBlob, blockTemplate, cb) {
        if (!global.config.verify_shares_host)
    ...
    this.get_miner_agent_warning_notification = function (agent) {
        ...
        if ((m = reXMRig.exec(agent))) {
            ...
            if (majorv + minorv < 30200) {
                return (
                    'Please update your XMRig miner (' +
                    agent +
                    ') to v3.2.0+ to support new rx/0 Monero algo'
                );
            }
            if (majorv + minorv >= 40000 && majorv + minorv < 40200) {
                return (
                    'Please update your XMRig miner (' +
                    agent +
                    ') to v4.2.0+ to support new rx/0 Monero algo'
                );
            }
            ...
            return (
                'Please update your xmr-stak miner (' +
                agent +
                ') to xmr-stak-rx miner to support new rx/0 Monero algo'
            );
    ...
    this.fixDaemonIssue = function (height, top_height, port) {
        ...
        if (fs.existsSync(fix_daemon_sh)) {
            child_process.exec(fix_daemon_sh + ' ' + port, function (error, stdout, stderr) {
                ...
                if (error)
                    console.error(
                        fix_daemon_sh + ' script returned error exit code: ' + error.code
                    );
            });
        } else {
