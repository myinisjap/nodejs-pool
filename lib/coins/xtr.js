...
function Coin(data) {
    ...
    this.getPortBlockHeaderByHash = function (port, blockHash, callback) {
        global.support.rpcPortDaemon(port, 'getblock', { hash: blockHash }, function (body) {
            ...
            const miner_tx_hash = body.result.miner_tx_hash || body.result.block_header.miner_tx_hash;

            if (body.result.hasOwnProperty('miner_tx_hash')) {
                global.support.rpcWallet('get_transfer_by_txid', { txid: miner_tx_hash }, function (body2) {
                    if (
    ...
    this.BlockTemplate = function (template) {
        ...
        if (merge_mining_ext.difficulties) {
            this.mergeMiningDifficulties = merge_mining_ext.difficulties;
        }
        ...
        this.isMergeMinedDifficultyReached = (coin, diff) => {
            let coinDiff = this.getMergeMinedAcheivedDifficulty(coin);
            if (!coinDiff) {
                return null;
            }
            return compareGte(diff, coinDiff);
        };

        this.getMergeMinedAcheivedDifficulty = (coin) => this.mergeMiningDifficulties[coin];
    };
