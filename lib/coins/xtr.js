...
function Coin(data) {
    ...
    this.getPortBlockHeaderByHash = function (port, blockHash, callback) {
        global.support.rpcPortDaemon(port, 'getblock', { hash: blockHash }, function (err, body) {
            if (err || typeof body === 'undefined' || !body.hasOwnProperty('result')) {
                console.error(JSON.stringify(body), err);
                return callback(true, body);
            }
            ...
            if (body.result.hasOwnProperty('miner_tx_hash')) {
                global.support.rpcWallet('get_transfer_by_txid', { txid: miner_tx_hash }, function (err, body2) {
                    if (
                        err ||
    ...
    this.BlockTemplate = function (template) {
        ...
        let auxData = getAuxData(template);
