...
function Database() {
    ...
    this.initEnv = function () {
        ...
        global.database.tariBlockDb = this.env.openDbi({
            name: 'tariblocks',
            create: true,
            integerKey: true,
            keyIsUint32: true,
    ...
    this.storeTariBlock = function (blockId, block) {
        const txn = global.database.env.beginTxn();
        if (txn.getBinary(global.database.tariBlockDb, blockId)) {
            throw new Error(`storeTariBlock: Cannot insert because block ${blockId} already exists`);
        }
        const data = global.protos.Block.encode(block);
        txn.putBinary(global.database.tariBlockDb, blockId, data);
        txn.commit();
    };
    ...
    this.getValidTariLockedBlocks = function () {
        this.refreshEnv();
        let txn = this.env.beginTxn({ readOnly: true });
        let cursor = new lmdb.Cursor(txn, this.tariBlockDb);
        let blockList = [];
        for (let found = cursor.goToFirst(); found; found = cursor.goToNext()) {
            cursor.getCurrentBinary(function (key, data) {
                // jshint ignore:line
                    ...
                    global.coinFuncs.getLastBlockHeader(function (err, body) {
                         ...
                         */
                            if (global.config.pps.enable) {
                                ...
                                callback(
                                    null,
                                    body.height - Math.floor(global.config.payout.blocksRequired / 2),
                                    Math.floor(body.difficulty * global.config.pplns.shareMulti * 2)
                                );
                            } else {
                            ...
                            if (body.height - oldestLockedBlockHeight > global.config.general.blockCleanWarning) {
                                global.support.sendEmail(
                                    ...
                                    'longRunner can not clean ' +
                                        (body.height - oldestLockedBlockHeight) +
                                        ' block from DB!'
                                );
                            }
                            /*
