'use strict';
const { sleep, tryParseMergeMiningAddress, toHex, db } = require('../common');
...
async function fetchDuePayouts() {
    const balances = await db.balance.findWhere('amount >= ? AND locked = 0', [
        support.decimalToCoin(config.payout.walletMin),
    ]);
    ...
    for (const balance of balances) {
        ...
        if (payout.isWithinThreshold()) {
            ...
            console.log(
                `[++] ${payout.tariAddress()} miner to bulk payment. Amount: ${support.coinToDecimal(payout.amount)}`,
            );
            pendingPayouts.push(payout);
        } else {
            console.log(
                `[+] Payout for user ${payout.tariAddress()} not completed because it falls below payout threshold (#{payout.payoutThreshold}`,
            );
        }
    }
...
async function executePayouts(payouts) {
    ...
    for (const result of results) {
        ...
        if (!payout) {
            throw new Error(
                `Wallet GRPC returned result for an address we didn't request! Address not found: ${result.tariAddress()}`,
            );
        }
        if (result.is_success) {
            ...
            await db.balance.updateCustom(payout.balanceId, 'locked = 1', []);
        } else {
...
function Payout(balanceId, addr, amount, poolType, paymentId) {
    ...
    const address = aux.tryParseMergeMiningAddress(addr);
    ...
    this.handleSuccessfulPayout = async (pendingPayout, transaction) => {
        console.log(
            `[*] Successful payment to ${pendingPayout.payment_address} of ${support.coinToDecimal(
                transaction.amount,
            )} XMR with tx_hash ${transaction.tx_id}`,
        );
        ...
        if (+result.affectedRows !== 1) {
            ...
            throw new Error(
                ...
                }, username = '${pendingPayout.tariAddress()}'`,
            );
        }
