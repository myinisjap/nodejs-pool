...
async function fetchDuePayouts(coin) {
    ...
    for (const balance of balances) {
        ...
        if (payout.isWithinThreshold()) {
            ...
            console.log(
                `[++] ${payout.username()} miner to bulk payment. Amount: ${support.coinToDecimal(payout.amount)}`,
            );
            pendingPayouts.push(payout);
        } else {
            console.log(
                `[+] Payout for user ${payout.username()} not completed because it falls below payout threshold (#{payout.payoutThreshold}`,
            );
        }
    }
...
async function executePayouts(payouts) {
    ...
    const recipients = payouts.map((p) => ({
        ...
        // TODO: Custom message
        message: `Pool payout ${p.balanceId}`,
    }));
    ...
    for (const result of results) {
        ...
        if (result.is_success) {
            ...
            console.warn(`Failed to submit payout for ${payout.username()}: '${result.failure_message}'.`);
    ...
    this.processPayouts = async () => {
        ...
        for (const transaction of resp.transactions) {
            ...
            if (transaction.is_cancelled) {
                ...
                await db.pendingPayouts.update(pendingPayout.id, { status: 'cancelled', last_checked_at: 'now()' });
                continue;
            }
...
async function main() {
    ...
    try {
        ...
        while (true) {
            ...
            debug(`${payouts.length} XTR payout(s) that are due to be paid.`);
            if (payouts.length > 0) {
