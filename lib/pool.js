...
function templateUpdate(repeating) {
    global.coinFuncs.getBlockTemplate(global.config.pool.address, function (rpcResponse) {
        if (rpcResponse) {
          ...
          console.error("getBlockTemplate RPC error: ", rpcResponse.error);
        ...
        if (repeating !== true) {
            setTimeout(templateUpdate, 3000);
        }
    });
}
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        case 'login':
            ...
            console.log("New miner registered: ID=" + minerId + " Port=" + miner.port);
            sendReply(null, {
                id: minerId,
                job: miner.getJob(),
                status: 'OK'
...
if (cluster.isMaster) {
    ...
    templateUpdate();
    setInterval(templateUpdate, 3000, true);
    ...
    async.each(global.config.ports, function (portData) {
        ...
        function socketConn(socket) {
            ...
            console.log("Client connected on "+ addr.address + ":" + addr.port);
