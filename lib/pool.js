...
function setNewBlockTemplate(template) {
    ...
    debug(`New active block template: height = ${template.height}, difficulty = ${template.difficulty}`);
...
function submit_block(
    ...
    global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [blockHex], (rpcResult, rpcStatus) => {
        ...
        if (rpcResult && rpcResult.result) {
            ...
            if (blockTemplate.isMergeMinedDifficultyReached('xtr', hashDiff)) {
                console.log('TARI DIFFICULTY ACHIEVED');
                global.database.storeTariBlock(blockTemplate.height, {
                    hash: blockFastHash,
                    difficulty: blockTemplate.difficulty,
                    shares: 0,
                    timestamp: Date.now(),
                    ...
                    valid: true,
                    extra: {
                        difficulty_xtr: blockTemplate.getMergeMinedAcheivedDifficulty('xtr'),
                        difficulty_xmr: blockTemplate.getMergeMinedAcheivedDifficulty('xmr'),
                    },
                });
            }

            if (blockTemplate.isMergeMinedDifficultyReached('xmr', hashDiff)) {
                console.log('MONERO DIFFICULTY ACHIEVED');
