...
function setNewBlockTemplate(template) {
    ...
    const tariDifficulty = activeTemplate.getAuxChainData('xtr');
    if (tariDifficulty) {
        debug(`New tari block template (tari difficulty = ${tariDifficulty})`);
    }
    ...
    global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [blockHex], (err, rpcResult, rpcStatus) => {
        ...
        if (rpcResult && rpcResult.result) {
            ...
            if (blockTemplate.isBaseDifficultyAchieved(hashDiff)) {
                debug(`Monero difficulty (${hashDiff}) achieved`);
            ...
            if (blockTemplate.isAuxChainDifficultyAchieved('xtr', hashDiff)) {
                let xtrData = blockTemplate.getAuxChainData('xtr');
                console.log('TARI DIFFICULTY ACHIEVED', xtrData);
                global.database.insertAltBlock(blockTemplate.height, {
                    hash: blockFastHash,
                    difficulty: blockTemplate.difficulty,
                    shares: 0,
                    timestamp: Date.now(),
                    ...
                    // TODO
                    port: 7878,
                    height: xtrData.height,
                    anchor_height: 0, //todo
                    value: xtrData.reward, //todo
                    pay_value: 0, //todo
                    pay_stage: '',
                    pay_status: '',
                    pay_ready: false,
                ...
                debug(`Did not achieve Tari difficulty (${hashDiff}) for block at height ${blockTemplate.height}`);
            }
