...
if (cluster.isMaster) {
    ...
    setInterval(function () {
        let trustedSharesPercent = (totalShares ? (trustedShares / totalShares) * 100 : 0).toFixed(
            2
        );
        let normalSharesPercent = (totalShares ? (normalShares / totalShares) * 100 : 0).toFixed(2);
        let invalidSharesPercent = (totalShares ? (invalidShares / totalShares) * 100 : 0).toFixed(
            2
        );
        let outdatedSharesPercent = (totalShares
            ? (outdatedShares / totalShares) * 100
            : 0
        ).toFixed(2);
        let throttledSharesPercent = (totalShares
            ? (throttledShares / totalShares) * 100
            : 0
        ).toFixed(2);
        console.log(
...
function registerPool() {
    global.mysql
        ...
        .then(function (rows) {
            rows.forEach(function (row) {
                if (row.ip !== global.config.bind_ip) {
                    console.error(
                        'Pool ID in use already for a different IP. Update MySQL or change pool ID.'
                    );
                    process.exit(1);
                }
            });
        ...
        .then(function () {
            ...
            global.mysql
                .query('DELETE FROM ports WHERE pool_id = ?', [global.config.pool_id])
                .then(function () {
...
function removeMiner(miner) {
    ...
    if (miner.payout in minerWallets && --minerWallets[miner.payout].count <= 0)
        delete minerWallets[miner.payout];
    ...
    global.coinFuncs.getPortBlockTemplate(port, function (body_bt) {
        if (!newCoinHashFactor[coin]) {
            console.log(
                'Aborting ' +
                    port +
                    ' last block template request because ' +
                    coin +
        ...
        if (body_bt) {
            const template = process_rpc_template(
                body_bt,
                coin,
                port,
                coinHashFactor,
...
function templateUpdate(coin, repeating) {
    ...
    debug(
        `coin=${coin}, port=${port}, coinHashFactor = ${coinHashFactor}, isMaster = ${cluster.isMaster}`
    );
    if (coinHashFactor) {
...
function anchorBlockUpdate() {
    if ('' in activeBlockTemplates && global.config.daemon.port === activeBlockTemplates[''].port)
        return;
    ...
    global.coinFuncs.getLastBlockHeader(function (err, body) {
        if (err === null) {
            ...
            console.error('Anchor last block header request failed!');
        }
    });
}
    ...
    if (isHashFactorChange) {
        ...
        if (cluster.isMaster)
            console.log(
                threadName +
                    strLogPrefix +
                    ' with hash factor changed to ' +
                    lastCoinHashFactorMM[coin]
            );
...
function setNewBlockTemplate(template) {
    ...
    if (coin in activeBlockTemplates) {
        if (activeBlockTemplates[coin].prev_hash === template.prev_hash) {
            if ('child_template' in template) {
                if (
                    ...
                    activeBlockTemplates[coin].child_template.prev_hash ===
                        template.child_template.prev_hash
                ) {
                    console.log(
                        threadName +
        ...
        if (activeBlockTemplates[coin].port != template.port && global.config.pool.trustedMiners)
            isExtraCheck = true;
    }
    if (cluster.isMaster) {
    ...
    debug(
        `New active block template: height = ${template.height}, difficulty = ${template.difficulty}`
    );
    ...
    setNewCoinHashFactor(
        template.isHashFactorChange,
        coin,
        template.coinHashFactor,
        isExtraCheck ? height : 0
    );
}
...
function getTargetHex(diff, size) {
    const size2 = size * 2;
    const diff2 = baseDiff
        .div(diff)
        .toBuffer({ endian: 'little', size: size })
        .toString('hex')
        .substr(0, size2);
...
function Miner(
    ...
    this.identifier =
        agent && agent.includes('MinerGate') ? 'MinerGate' : pass_split[0].substring(0, 64);
    ...
    this.logString =
        this.payout.substr(this.payout.length - 10) +
        ':' +
        this.identifier +
        ' (' +
        ipAddress +
    ...
    for (let index = 1; index < login_div_split.length - 1; index += 2) {
        ...
        if (isNaN(percent) || percent < 0.1) {
            this.error =
                'Your payment divide split ' + percent + " is below 0.1% and can't be processed";
            this.valid_miner = false;
            return;
        }
        if (percent > 99.9) {
            this.error =
                'Your payment divide split ' + percent + " is above 99.9% and can't be processed";
            this.valid_miner = false;
            return;
        }
        ...
        if (address in bannedAddresses) {
            // Banned Address
            this.error =
                'Permanently banned payment address ' +
                address +
                ' provided: ' +
    ...
    this.setAlgos = function (algos, algos_perf, algo_min_time) {
        ...
        if (global.coinFuncs.is_miner_agent_no_haven_support(this.agent))
            delete this.algos['cn-heavy/xhv'];
        ...
        if (!(algos_perf && algos_perf instanceof Object)) {
            if (global.coinFuncs.algoMainCheck(this.algos))
        ...
        if (coin_perf instanceof Object) {
            if (!('' in coin_perf && global.coinFuncs.algoMainCheck(this.algos)))
                coin_perf[''] = -1;
    ...
    this.wallet_key =
        ...
        this.poolTypeEnum +
        ' ' +
        JSON.stringify(this.payout_div) +
        ' ';
    ...
    if (email != '') {
        ...
        if (
            !(payoutAddress in walletLastCheckTime) ||
            time_now - walletLastCheckTime[payoutAddress] > 60 * 1000
        ) {
            global.mysql
                ...
                .then(function (rows) {
                    ...
                    global.mysql.query('INSERT INTO users (username, email) VALUES (?, ?)', [
                        payoutAddress,
                        email,
                    ]);
    ...
    this.setNewDiff = function (difficulty) {
        ...
        debug(
            threadName + 'Difficulty change to: ' + this.newDiffToSet + ' For: ' + this.logString
        );
        if (this.hashes > 0) {
            debug(
                threadName +
    ...
    this.selectBestCoin = function () {
        if (this.debugMiner)
            console.log(threadName + this.logString + ': current coin is ' + this.curr_coin);
        if (
        ...
        COINS.forEach(function (coin) {
            if (!(coin in miner.coin_perf)) {
                if (miner.debugMiner)
                    console.log(threadName + miner.logString + ': ' + coin + ': no coin_perf');
                return;
            }
            if (!(coin in activeBlockTemplates)) {
                if (miner.debugMiner)
                    console.log(
                        threadName + miner.logString + ': ' + coin + ': no activeBlockTemplates'
                    );
                return;
            }
            ...
            if (!coinHashFactor) {
                if (miner.debugMiner)
                    console.log(threadName + miner.logString + ': ' + coin + ': no coinHashFactor');
                return;
            }
            ...
            const factor =
                (typeof miner.curr_coin_hash_factor === 'undefined'
                    ? 1
                    : miner.curr_coin_hash_factor) / coinHashFactor;
            ...
            if (!global.coinFuncs.isMinerSupportAlgo(algo, miner.algos)) {
                if (miner.debugMiner)
                    console.log(threadName + miner.logString + ': ' + coin + ': no algo support');
                return;
            }
            let coin_perf = miner.coin_perf[coin] * coinHashFactor;
            if (miner.curr_coin === coin) coin_perf *= 1.05;
            if (miner.debugMiner)
                console.log(threadName + miner.logString + ': ' + coin + ': ' + coin_perf);
        ...
        if (typeof this.curr_coin === 'undefined' || this.curr_coin != best_coin) {
            const blob_type_num = global.coinFuncs.portBlobType(
                global.coinFuncs.COIN2PORT(best_coin)
            );
            ...
            const curr_hash_factor =
                typeof this.curr_coin_hash_factor === 'undefined' ? 1 : this.curr_coin_hash_factor;
            ...
            if (global.config.pool.trustedMiners)
                this.trust.check_height = activeBlockTemplates[best_coin].height;
        }
        return best_coin;
    };
    ...
    this.checkBan = function (validShare) {
        ...
        if (validShare) {
            ...
            if (this.validShares === 0) {
                console.error(
                    threadName +
                        'Suspended miner IP for submitting bad share with zero trust ' +
                        this.logString
                );
                removeMiner(this);
        ...
        if (shareCount >= global.config.pool.banThreshold) {
            if ((100 * this.invalidShares) / shareCount >= global.config.pool.banPercent) {
                console.error(
                    threadName +
                        'Suspended miner IP for submitting too many bad shares recently ' +
                        this.logString
                );
                removeMiner(this);
                process.send({ type: 'banIP', data: this.ipAddress });
    ...
    if (protoVersion === 1) {
            ...
            if (!this.proxy) {
                ...
                if (isGrin)
                    ...
                    this.cachedJob = {
                        ...
                        target: getTargetHex(
                            this.difficulty,
                            global.coinFuncs.nonceSize(blob_type_num)
                        ),
                        id: this.id,
                    };
            } else {
        ...
        this.sendSameCoinJob = function () {
            const coin =
                typeof this.curr_coin !== 'undefined' ? this.curr_coin : this.selectBestCoin();
...
function recordShareData(miner, job, isTrustedShare, blockTemplate) {
    ...
    const db_job_height =
        global.config.daemon.port == blockTemplate.port ? blockTemplate.height : anchorBlockHeight;

    let wallet = walletAcc[wallet_key];
    const worker_name =
        miner.identifier in wallet || walletWorkerCount[wallet_key] < 50
            ? miner.identifier
            : 'all_other_workers';
...
function submit_block(
    ...
    const onError = (rpcResult, rpcStatus) => {
        ...
        if (isNotifyAdmin) {
            setTimeout(function () {
                ...
                global.coinFuncs.getPortLastBlockHeader(blockTemplate.port, function (err, body) {
                    if (err !== null) {
                        console.error(
                            'Last block header request failed for ' + blockTemplate.port + ' port!'
                        );
                        return;
                    }
    ...
    global.support.rpcPortDaemon(
        ...
        (rpcResult, rpcStatus) => {
            ...
            if (rpcResult && (rpcResult.error || rpcResult.result === 'high-hash')) {
                onError(rpcResult, rpcStatus);
            ...
            if (rpcResult && rpcResult.result) {
                const blockFastHash = global.coinFuncs
                    .getBlockID(shareBuffer, blockTemplate.port)
                    .toString('hex');
                console.log(
                    threadName +
                        'New ' +
                ...
                global.database.storeBlock(
                    ...
                    global.protos.Block.encode({
                        ...
                        valid: true,
                        extra: {
                            difficulty_tari: blockTemplate.tariDifficulty,
                            difficulty_monero: blockTemplate.moneroDifficulty,
                        },
                    })
                );
...
function report_miner_share(miner, job) {
    const time_now = Date.now();
    if (
        !(miner.payout in lastMinerLogTime) ||
        time_now - lastMinerLogTime[miner.payout] > 30 * 1000
    ) {
        console.error(
            threadName + 'Bad share from miner (diff ' + job.difficulty + ') ' + miner.logString
        );
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    let verifyShare = function (verifyShareCB) {
        ...
        debug(
            `Verify share: achieved difficulty: ${hashDiff}, target: ${blockTemplate.difficulty}`
        );
        ...
        if (
            ...
            if (miner.payout in extra_wallet_verify) {
                ...
                if (shareBuffer !== null) {
                    ...
                    console.error(
                        'EXTRA WALLET VERIFY ' + miner.payout + ": CAN'T MAKE SHARE BUFFER"
                    );
                }
            }
            ...
            if (
                ...
                ++minerWallets[miner.payout].last_ver_shares >=
                    MAX_VER_SHARES_PER_SEC * VER_SHARES_PERIOD
            ) {
                if (
                    minerWallets[miner.payout].last_ver_shares ===
                    MAX_VER_SHARES_PER_SEC * VER_SHARES_PERIOD
            ...
            if (blockTemplate.isDifficultyReached(hashDiff)) {
                debug(
                    `miner:${miner.payout} Share reached target difficulty for block ${blockTemplate.height}`
                );
                if (
                    miner.validShares ||
                    (miner.payout in minerWallets && minerWallets[miner.payout].hashes)
                ) {
                    submit_block(
                        miner,
                        job,
                        ...
                        function (block_submit_result) {
                            if (!block_submit_result) {
                                const hash = global.coinFuncs.slowHash(
                                    convertedBlob,
                                    blockTemplate
                                );
                                if (hash !== resultHash) {
                ...
                debug(`Share was below target difficulty for block ${blockTemplate.height}`);
                const time_now = Date.now();
                global.coinFuncs.slowHashAsync(convertedBlob, blockTemplate, function (hash) {
                    if (hash === null) {
                        return processShareCB(null);
    ...
    verifyShare(function (hashDiff, shareBuffer, isTrustedShare, isBlockDiffReached) {
        if (isBlockDiffReached === null && ge(hashDiff, blockTemplate.difficulty)) {
    ...
    switch (method) {
        ...
        case 'login': {
            ...
            if (!miner.valid_miner) {
                if (
                    !(miner.payout in lastMinerLogTime) ||
                    time_now - lastMinerLogTime[miner.payout] > 10 * 60 * 1000
                ) {
                    console.log(
                        'Invalid miner ' +
                            miner.logString +
            ...
            const miner_agent_notification =
                !global.coinFuncs.algoMainCheck(miner.algos) &&
                global.coinFuncs.algoPrevMainCheck(miner.algos)
            ...
            if (miner_notification) {
                if (
                    ...
                    console.error(
                        'Sent notification to ' + miner.logString + ': ' + miner_notification
                    );
                    sendReplyFinal(
                        miner_notification + ' (miner will connect after several attempts)'
                    );
                    return;
                }
            }
            ...
            if (!miner.proxy) {
                let proxyMinerName = miner.payout; // + ":" + miner.identifier;
                if (
                    (params.agent && params.agent.includes('proxy')) ||
                    proxyMinerName in proxyMiners
                ) {
                    addProxyMiner(miner);
            ...
            if (id === 'Stratum') {
                ...
                if (coin !== false) {
                    ...
                    const blob_type_num = global.coinFuncs.portBlobType(
                        global.coinFuncs.COIN2PORT(coin)
                    );
        ...
        case 'getjobtemplate': {
            // grin only
            const minerId =
                socket.miner_ids && socket.miner_ids.length == 1 ? socket.miner_ids[0] : '';
        ...
        case 'submit': {
            ...
            const nonce_sanity_check = function (blob_type_num, params) {
                if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
                    ...
                    if (params.pow.length != global.coinFuncs.c29ProofSize(blob_type_num))
                        return false;
                } else {
            ...
            processShare(miner, job, blockTemplate, params, function (shareAccepted) {
                ...
                if (shareAccepted === null) {
                    sendReply(
                        'Throttled down share submission (please use high fixed diff or use xmr-node-proxy)'
                    );
                    return;
                }
        ...
        global.mysql
            ...
            .then(function (rows) {
                ...
                if (top_height) {
                    ...
                    console.error(
                        "Can't get top height amongst all leaf nodes for " + port + ' port'
                    );
                    lastBlockFixTime[port] = Date.now();
                    lastBlockFixCount[port] = 0;
                }
    ...
    function add_bans(is_show) {
        global.mysql.query('SELECT mining_address, reason FROM bans').then(function (rows) {
            ...
            rows.forEach(function (row) {
                ...
                if (is_show)
                    console.log('Added blocked address ' + row.mining_address + ': ' + row.reason);
            });
        });
        global.mysql
            .query('SELECT mining_address, message FROM notifications')
            .then(function (rows) {
                notifyAddresses = {};
                rows.forEach(function (row) {
                    ...
                    if (is_show)
                        console.log(
                            'Added notify address ' + row.mining_address + ': ' + row.message
                        );
                });
            });
    }
    ...
    for (let i = 1; i <= numWorkers; ++i) {
        ...
        lineReader.on('line', function (line) {
            ...
            if (
                ...
                (!(wallet in walletTrust) || trust < walletTrust[wallet])
            ) {
    ...
    setInterval(function () {
        ...
        for (let wallet in walletTrust) {
            ...
            if (Date.now() - time < 24 * 60 * 60 * 1000) {
                str +=
                    wallet + '\t' + walletTrust[wallet].toString() + '\t' + time.toString() + '\n';
            } else {
            ...
            socket
                .on('data', function (d) {
                    ...
                    if (Buffer.byteLength(dataBuffer, 'utf8') > 102400) {
                        ...
                        dataBuffer = null;
                        console.warn(
                            threadName + 'Excessive packet size from: ' + socket.remoteAddress
                        );
                        socket.destroy();
                        return;
                    }
                ...
                .on('close', function () {
                    ...
                    if (socket.miner_ids)
                        socket.miner_ids.forEach((miner_id) => activeMiners.delete(miner_id));
                });
        }
